...
// ゲーム内で同じ遺物が2つの名前を持ちうるもの（幽霊カード表示時のみ1枚にまとめる。実在するカードは名前を変えず、まとめない）
const SPECIAL_ITEM_NAME_GROUPS = [["魔女のブローチ", "砕けた魔女のブローチ"]];
...
function foldGenericLayers(rows) {
  ...
  const ghostSpecialCards = useMemo(() => {
    ...
    const groupedNames = new Set(SPECIAL_ITEM_NAME_GROUPS.flat());
    const ghosts = [];

    // 別名グループ：グループ内のどれか1つでも所持していれば、幽霊カードは出さない
    SPECIAL_ITEM_NAME_GROUPS.forEach((names) => {
      if (names.some((n) => ownedNames.has(n))) return;
      const info = SPECIAL_ITEMS_DB[names[0]];
      if (!info) return;
      ghosts.push({
        __ghost: true,
        id: `ghost-${names.join("-")}`,
        name: names.join("／"),
        effectiveColor: info.color,
        skills: info.skills,
      });
    });

    // 別名グループに属さない、通常の固有遺物
    Object.entries(SPECIAL_ITEMS_DB).forEach(([name, info]) => {
      if (groupedNames.has(name) || ownedNames.has(name)) return;
      ghosts.push({ __ghost: true, id: `ghost-${name}`, name, effectiveColor: info.color, skills: info.skills });
    });
